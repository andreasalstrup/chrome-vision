...
class ChromeCutter:
    def __init__(self, annotations_file, img_dir, name):
        if os.path.exists(f"data/leftImg8bit/train/cut/{name}"):
            return
        numOfCuts = 0
        if not(os.path.exists("data/leftImg8bit/train/cut")):
            os.mkdir("data/leftImg8bit/train/cut")
        os.mkdir(f"data/leftImg8bit/train/cut/{name}")
        ...
        for i in range(len(img_names)):
            ...
            for result in cuts.xyxy[0]:                
                ...
                cv2.imwrite(f"data/leftImg8bit/train/cut/{numOfCuts}", croppedImage)
                numOfCuts += 1
